...
export type StockfishStatus = 'loading' | 'ready' | 'thinking' | 'error';
...
export function useStockfish() {
  ...
  const statusRef = useRef<StockfishStatus>('loading');
  ...
  const getBestMove = useCallback(
    ...
    [sendCommand, restartWorker]
  );

  const evaluatePosition = useCallback(
    async (fen: string, depth: number): Promise<number | null> => {
      const move = await getBestMove(fen, { depth, movetime: 500 });
